            ...
            div(
                ...
                div(
                    ...
                    tagAppendAttributes(
                        fileInput(
                            inputId = ns(
                                id_list()$data_import$isa_opt_section$inputs[[
                                  "json_file_input"]]
                            ),
...
dataImport.metadataServer <- function(id, workflow, af_file_cols) {
    moduleServer(
        ...
        function(input, output, session) {
            ...
            to_return$metadata <- reactive({
                metadata()
            })
            to_return$fs_aligned_af <- reactive({
                fs_aligned_af()
            })
            to_return$ind_sample_id <- reactive({
                input[[
                    id_list()$data_import$metadata_section$inputs$sample_id
                ]]
            })
